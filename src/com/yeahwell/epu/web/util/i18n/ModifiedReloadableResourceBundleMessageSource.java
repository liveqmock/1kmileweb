 ...
 */
public class ModifiedReloadableResourceBundleMessageSource extends
	...
	private String[] extensions = (String[]) Arrays.asList(PROPERTIES_SUFFIX,
			XML_SUFFIX).toArray();
	...
	public void setPropertiesPersister(
			final PropertiesPersister propertiesPersister) {
	...
	protected String resolveCodeWithoutArguments(final String code,
		...
		if (this.cacheMillis < 0) {
			...
			for (final String basename : this.basenames) {
				final List<String> filenames = calculateAllFilenames(basename,
						locale);
	...
	protected MessageFormat resolveCode(final String code, final Locale locale) {
		if (this.cacheMillis < 0) {
			final PropertiesHolder propHolder = getMergedProperties(locale);
			final MessageFormat result = propHolder.getMessageFormat(code,
					locale);
			if (result != null) {
				return result;
			}
			...
			for (final String basename : this.basenames) {
				...
				for (final String filename : filenames) {
					final PropertiesHolder propHolder = getProperties(filename);
					final MessageFormat result = propHolder.getMessageFormat(
							code, locale);
					if (result != null) {
						return result;
					}
	...
	protected List<String> calculateAllFilenames(final String basename,
		...
		synchronized (this.cachedFilenames) {
			...
			if (this.fallbackToSystemLocale
				...
				final List<String> fallbackFilenames = calculateFilenamesForLocale(
	...
	protected PropertiesHolder getProperties(final String filename) {
		synchronized (this.cachedProperties) {
			final PropertiesHolder propHolder = this.cachedProperties
					.get(filename);
			if ((propHolder != null)
					&& ((propHolder.getRefreshTimestamp() < 0) || (propHolder
							.getRefreshTimestamp() > (System
							.currentTimeMillis() - this.cacheMillis)))) {
				// up to date
				return propHolder;
			}
	...
	protected PropertiesHolder refreshProperties(final String filename,
		...
		final long refreshTimestamp = (this.cacheMillis < 0 ? -1 : System
				.currentTimeMillis());
	...
	public void clearCacheIncludingAncestors() {
		...
		if (getParentMessageSource() instanceof ModifiedReloadableResourceBundleMessageSource) {
			((ModifiedReloadableResourceBundleMessageSource) getParentMessageSource())
	...
	protected class PropertiesHolder {
		...
		public MessageFormat getMessageFormat(final String code,
				final Locale locale) {
			if (this.properties == null) {
				return null;
			}
			synchronized (this.cachedMessageFormats) {
				...
				if (msg != null) {
					...
					final MessageFormat result = createMessageFormat(msg,
							locale);
