...
public class KEFileManagerController extends BaseController {

	private Logger logger = LoggerFactory
			.getLogger(KEFileManagerController.class);

	@RequestMapping(value = "/fileManager", method = { RequestMethod.GET,
			RequestMethod.POST })
	public void fileManager(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		logger.debug("进入KindEditor的FileManager");
		PrintWriter writer = response.getWriter();
		ServletContext servletContext = request.getSession()
				.getServletContext();
		// 根目录路径，可以指定绝对路径，比如 /var/www/attached/
		String rootPath = servletContext.getRealPath("/") + "attached/";
